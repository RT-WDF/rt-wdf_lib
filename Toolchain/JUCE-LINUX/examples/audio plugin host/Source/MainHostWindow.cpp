...
private:
    ...
    static void initAudioType( AudioDeviceManager* deviceManager )
    {
        int id = 1;
        // Get default Audio device type
        String defaultAudioType = deviceManager->getCurrentAudioDeviceType();


        // iterate all audio type options
        for (int i = 0; i < deviceManager->getAvailableDeviceTypes().size(); ++i)
        {
            ...
            if (typeName == "JACK")
            {
                // .. and set it as the default audio device. (this calls prepareToPlay in turn and sets T = 1/fs etc properly !!)
                deviceManager->setCurrentAudioDeviceType("JACK", true);

                // check actual device to see if it's not empty..
                AudioIODevice* device = deviceManager->getCurrentAudioDevice();

                // .. and there are actually any outputs (which means that jackd is running)
                if (device)
                {
                    // if jack is running properly and we have actual outputs, set JACK as default
                }
                // if not: reset Audio type back to default and disable JACK entry in the combo box
                else
                {
                    deviceManager->setCurrentAudioDeviceType(defaultAudioType, true);
                }
            }
